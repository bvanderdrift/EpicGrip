    ...
    class GameEngine
    {
        ...
        static public void Update()
        {
            ...
            for (int i = 0; i < carsInGame; i++)
            {
                ...
                {
                    cars[i].CheckCollision(ref cars[j], 50);
                }
            }
            //Update the cars
            for (int i = 0; i < carsInGame; i++)
            ...
            if (totalLossCars >= carsInGame - 1 && engineState != EngineState.Ended)
            {
                for (int i = 0; i < carsInGame; i++)
                {
                    ...
                    {
#if XBOX
                        End(InfoPacket.PlayerProfiles[(int)InfoPacket.Players[i]].Gamertag);
#endif
            ...
            if (engineState == EngineState.Ended)
            {
                for (int i = 0; i < carsInGame; i++)
                {
...
#if XBOX
                    endingTexts[i] = InfoPacket.PlayerProfiles[(int)InfoPacket.Players[i]].Gamertag + ":" + "\n" +
                        "Time Alive: " + (int)(cars[i].TimeAlive / 1000) + " seconds" + "\n" +
                        "Damage Dealt: " + cars[i].DamageDealt + "\n" +
                        "Winner Bonus: " + "\n" +
                        "Money Gained: " + "\n" +
                    ...
                    moneyGainedTexts[i] = "\n" +
                        ...
                        "+" + cars[i].DamageDealt.ToString() + "\n" +
                        "+" + bonus + "\n" +
                        ((int)((cars[i].TimeAlive / totalTimePlayed) * 100) + cars[i].DamageDealt + bonus).ToString() + "\n" +
                        InfoPacket.PlayerStatistics[i].Money.ToString();
#endif
                }
            }
        ...
        static public void End(string winner)
        {
            ...
            for (int i = 0; i < carsInGame; i++)
            {
                InfoPacket.PlayerStatistics[i].Money += (int)(((float)cars[i].TimeAlive / (float)totalTimePlayed) * 100) + cars[i].DamageDealt + cars[i].Health;
            }

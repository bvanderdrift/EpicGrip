...
namespace PWS.TheGame
{
    ...
    class Car
    {
        ...
        public void Update()
        {
            ...
            GamePadState state = GamePad.GetState((PlayerIndex)playerNumber);
            ...
            if (state.Triggers.Right > .3f && InfoPacket.PreviousStates[playerNumber].Triggers.Right <= .3f && GameEngine.State == EngineState.Playing)
            {
            ...
            if (collisionPosition != Vector2.Zero)
            {
                ...
                if (speed > 0)
                {
                    if (relativeAngleOfImpact < frontCornersAngle || relativeAngleOfImpact > (Math.PI * 2) - frontCornersAngle)
                    {
                        ...
                        if (speed > .5f)
                        {
                            ...
                            damageDealt += otherCar.lastHealth - otherCar.currentHealth;
                ...
                if (otherCar.speed > 0)
                {
                    if (relativeAngleOfImpact < frontCornersAngle || relativeAngleOfImpact > (Math.PI * 2) - frontCornersAngle)
                    {
                        ...
                        if (otherCar.speed > .5f)
                        {
                            ...
                            otherCar.damageDealt += lastHealth - currentHealth;
                ...
                else if (otherCar.speed < 0)
                {
                    if (relativeAngleOfImpact < frontCornersAngle + Math.PI && relativeAngleOfImpact > (Math.PI * 2) - frontCornersAngle + Math.PI)
                    {
                        ...
                        if (otherCar.speed < -.5f)
                        {
                            otherCar.damageDealt += Math.Min((int)(otherCar.damageFactor * Math.Abs(otherCar.speed)), currentHealth);
                            DecreaseHealth((int)(otherCar.damageFactor * Math.Abs(otherCar.speed)));
